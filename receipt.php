...
if (!isset($_GET['sale_group_id']) || !is_numeric($_GET['sale_group_id'])) {
    die("Invalid invoice request.");
}

$sale_group_id = (int) $_GET['sale_group_id'];
if ($sale_group_id <= 0) {
    die("Invalid invoice request.");
}
...
$stmt = $conn->prepare("SELECT id, username FROM users WHERE id = ? AND role = 'staff'");
if ($stmt) {
...
if (empty($sale_items)) {
    die("No sales items found for this invoice.");
}
...
$created_at      = $sale_group['created_at'] ?? $date;
$customer_name   = $sale_group['customer_name'] ?? '';
$processed_by    = $sale_group['processed_by'] ?? $sale_group['created_by'] ?? $cashierName;
    ...
    <style>
        * {
        ...
        .invoice-container {
            ...
            min-height: 940px; /* Letter height optimization */
            display: flex;
...
<div class="invoice-container">
    ...
    <div class="table-wrap">
        <table class="invoice-table">
            ...
            <tbody>
                ...
                ?>
                <tr class="item-row">
                    <td style="text-align:center; font-family: 'IBM Plex Mono';">
                        <?php 
                        $q = (float)$item['quantity'];
                        echo ($q == (int)$q) ? (int)$q : rtrim(rtrim(number_format($q, 4), '0'), '.');
                        ?>
                    </td>
                    <td style="text-align:center; text-transform:uppercase; font-size:10px;"><?= htmlspecialchars($item['unit'] ?? '-') ?></td>
                    <td>
    ...
    <div class="invoice-footer">
        ...
        <div class="signature-area">
            <div class="sig-line"><?= strtoupper(htmlspecialchars($processed_by ?: 'CASHIER')) ?></div>
...
<div class="action-bar no-print">
    ...
    <a href="purchased_history.php" class="btn-modern">
        Back to Sales List
    </a>
</div>
